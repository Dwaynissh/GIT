...
export const createProject = async (req: Request, res: Response) => {
    try {
        ...
        const company = await todoModel.findById(companyID);
        const allStakeholders = await todoModel.findById(staffID)
...
export const readProject = async (req: Request, res: Response) => {
    try {
      ...
      const company = await todoModel.findById(companyID).populate({
        path: "project",
        options: {
          sort: {
            createdAt: -1,
